...
class BlackJack(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.name_player_label.place(x=475, y=668, width=140)
        ...
        try:
            with open('save.txt', 'r') as file:
                ...
                self.chips_player = int(self.save_dict["chips"])
                self.chips_player_label.configure(text=f"{self.chips_player} $")
                self.name_player_label.configure(text=self.save_dict["name"])
        except:
            pass
    ...
    def save_data_player(self):
        new_save_dict = {"name": self.save_dict["name"], "chips": self.chips_player, "gender": self.save_dict["gender"]}
